...
//        if(Common.questionList.size() != 0)
        ...
        btn_filter_no_answer.setOnClickListener(v -> {
        ...
        btn_filter_wrong.setOnClickListener(v -> {
            Common.answerSheetListFiltered.clear();
            for(int i = 0; i < Common.answerSheetList.size(); i++)
            {
        ...
        btn_filter_right.setOnClickListener(v -> {
            Common.answerSheetListFiltered.clear();
            for(int i = 0; i < Common.answerSheetList.size(); i++)
            {
        ...
        DatabaseReference reference  = FirebaseDatabase.getInstance().getReference("GuessEd");
...
//                Toast.makeText(ResultActivity.this, "There 3", Toast.LENGTH_SHORT).show();
            ...
            case R.id.menu_high_score:
                if(OnlineDBHelper.getInstance(this, FirebaseDatabase.getInstance()) != null)
                {
                    ...
                    MaterialDialog mDialog = new MaterialDialog.Builder(ResultActivity.this)
                            .setTitle("Hi "+name)
                            .setMessage("Your current high score in class "+Common.selectedCategory.getClassId()+" of "+Common.selectedCategory.getName()+" category is "+highscoretodisplay+"%")
                            .setPositiveButton("OK", R.drawable.ic_mood_white_24dp, (dialogInterface, which) -> dialogInterface.dismiss())
                            .build();
                    mDialog.show();
                }
                break;
...
//                                btn_sign_out.setEnabled(false);
                        ...
                        }).addOnFailureListener(e -> Toast.makeText(ResultActivity.this, ""+e.getMessage(), Toast.LENGTH_SHORT).show());
                break;
